...
document.querySelector('.left-y-control')
    .addEventListener('click', () => {
        cube.style.transform = 
            `rotate(${x}deg) rotateY(${y -= 20}deg) rotateZ(${z}deg)`;
    });
...
document.querySelector('.top-z-control')
    .addEventListener('click', () => {
        cube.style.transform = 
            `rotate(${x}deg) rotateY(${y}deg) rotateZ(${z -= 20}deg)`;
    });
...
const playPause = () => {
    if(bool) {
        interval = setInterval(() => {
            cube.style.transform = 
                `rotate(${x}deg) rotateY(${y++}deg) rotateZ(${z}deg)`;
        }, 100); 
    } else {
...
const section3Content = 
    document.querySelector('.section-3-content');
